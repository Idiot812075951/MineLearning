...
REFERENCE_IMAGE = r"C:\Users\gh\Documents\Unreal Projects\MineLearning\ArtSource\Environment\RobotCenter\References\RobotCenter_P03_PathReference.png"
...
class MeshAccumulator:
    ...
    def add_x_ramp_wedge(self, x_outer, x_inner, y_center, width, z_outer, z_inner, material_index):
        hy = width * 0.5
        verts = [
            (x_outer, y_center - hy, 0.0), (x_inner, y_center - hy, 0.0),
            (x_inner, y_center + hy, 0.0), (x_outer, y_center + hy, 0.0),
            (x_outer, y_center - hy, z_outer), (x_inner, y_center - hy, z_inner),
            (x_inner, y_center + hy, z_inner), (x_outer, y_center + hy, z_outer),
        ]
        faces = [
            (0, 3, 2, 1), (4, 5, 6, 7),
            (0, 1, 5, 4), (1, 2, 6, 5),
...
def create_mesh_object(name, accumulator, collection, materials, material_order, origin, bevel=0.08):
    ...
    obj["RC_Phase"] = "P03_LayoutOptimization"
...
def create_reference_proxies(collection, materials):
    ...
    robot_acc.add_box((-5.15, -1.75, 1.79), (0.70, 0.62, 0.72), 0)
    robot_acc.add_box((-5.15, -1.75, 2.32), (0.56, 0.50, 0.42), 0)
    robot = create_mesh_object(
        "REF_OreBuddy_1p1m", robot_acc, collection, materials,
        ["M_RC_ProxyRobot"], (-5.15, -1.75, 1.43), bevel=0.09,
    )
...
def create_preview_setup(scene, collection, materials):
    ...
    floor_acc.add_cylinder((-0.75, 0.0), 12.5, -0.12, -0.08, 0, segments=64)
...
def build():
    ...
    scene.name = "RobotCenter_P03_Layout"
    ...
    # 03 Shop Terminal: kept inside the left work zone, clear of the robot exit lane.
    shop_acc = MeshAccumulator()
    shop_center = Vector((-3.45, 0.68, 0.0))
    shop_acc.add_chamfered_prism((-3.45, 0.68), 2.55, 1.62, 0.78, 1.16, 0.24, mat_index["M_RC_Gunmetal"])
    shop_acc.add_chamfered_prism((-3.45, 0.68), 2.30, 1.42, 1.10, 2.56, 0.22, mat_index["M_RC_Gunmetal"])
    shop_acc.add_box((-3.45, 0.66, 2.63), (2.54, 1.58, 0.40), mat_index["M_RC_Yellow"])
    shop_acc.add_box((-3.45, -0.02, 2.08), (1.74, 0.16, 0.88), mat_index["M_RC_Gunmetal"])
    shop_acc.add_box((-3.45, -0.13, 2.08), (1.26, 0.07, 0.56), mat_index["M_RC_Cyan"])
    shop_acc.add_box((-3.45, -0.10, 1.34), (1.42, 0.58, 0.42), mat_index["M_RC_Yellow"], rotation=(-0.20, 0.0, 0.0))
    shop_acc.add_box((-3.45, -0.37, 1.47), (1.04, 0.12, 0.22), mat_index["M_RC_Gunmetal"], rotation=(-0.20, 0.0, 0.0))
    shop_rotation = Matrix.Translation(shop_center) @ Matrix.Rotation(math.radians(6.0), 4, "Z") @ Matrix.Translation(-shop_center)
    shop_acc.transform(shop_rotation)
    create_mesh_object("SM_RobotCenter_ShopTerminal", shop_acc, collections["03_RobotShop"], materials, material_order, (-3.45, 0.68, 0.78), bevel=0.11)

    # 04 Robot deployment pad: moved left/outward; its left edge is the robot-only exit side.
    shop_pad_acc = MeshAccumulator()
    shop_pad_center = (-5.15, -1.75)
    shop_pad_acc.add_chamfered_prism(shop_pad_center, 3.18, 2.78, 0.78, 1.08, 0.34, mat_index["M_RC_Gunmetal"])
    shop_pad_acc.add_chamfered_prism(shop_pad_center, 2.90, 2.50, 1.08, 1.27, 0.30, mat_index["M_RC_Yellow"])
    shop_pad_acc.add_chamfered_prism(shop_pad_center, 2.48, 2.08, 1.27, 1.35, 0.24, mat_index["M_RC_Cyan"])
    shop_pad_acc.add_chamfered_prism(shop_pad_center, 2.20, 1.80, 1.35, 1.43, 0.20, mat_index["M_RC_Gunmetal"])
    for y in (-2.68, -0.82):
        shop_pad_acc.add_box((-3.88, y, 1.49), (0.38, 0.38, 0.24), mat_index["M_RC_Gunmetal"])
    shop_pad_acc.add_box((-4.22, -0.47, 1.08), (1.10, 0.36, 0.22), mat_index["M_RC_Gunmetal"])
    shop_pad = create_mesh_object("SM_RobotCenter_ShopDisplayPad", shop_pad_acc, collections["03_RobotShop"], materials, material_order, (-5.15, -1.75, 0.78), bevel=0.08)
    shop_pad["RC_RobotExitSide"] = "-X"
    ...
    # 06 Scanner: three independent low nodes behind/outside the pad. No arch, top bar, or cage.
    scanner_acc = MeshAccumulator()
    scanner_nodes = ((1.13, 0.02), (5.51, 0.02), (3.32, 1.30))
    for node_x, node_y in scanner_nodes:
        dx = form_center[0] - node_x
        dy = form_center[1] - node_y
        direction = Vector((dx, dy)).normalized()
        yaw = math.atan2(-direction.x, direction.y)
        scanner_acc.add_chamfered_prism((node_x, node_y), 0.82, 0.78, 0.78, 1.12, 0.14, mat_index["M_RC_Gunmetal"])
        scanner_acc.add_box((node_x, node_y, 1.24), (0.92, 0.76, 0.30), mat_index["M_RC_Yellow"], rotation=(0.0, 0.0, yaw))
        head_center = (node_x + direction.x * 0.26, node_y + direction.y * 0.26, 1.47)
        lens_center = (node_x + direction.x * 0.55, node_y + direction.y * 0.55, 1.47)
        scanner_acc.add_box(head_center, (0.62, 0.52, 0.40), mat_index["M_RC_Gunmetal"], rotation=(0.0, 0.0, yaw))
        scanner_acc.add_box(lens_center, (0.40, 0.08, 0.24), mat_index["M_RC_Cyan"], rotation=(0.0, 0.0, yaw))
    scanner = create_mesh_object("SM_RobotCenter_FormScanner", scanner_acc, collections["04_TransformStation"], materials, material_order, (3.32, 0.20, 0.78), bevel=0.08)
    scanner["RC_ScannerNodeCount"] = 3
    scanner["RC_OpenFront"] = True

    # 07 Form Terminal: side-front placement keeps the approach and pad face unobstructed.
    form_terminal_acc = MeshAccumulator()
    form_terminal_center = (6.12, -1.92)
    form_terminal_acc.add_chamfered_prism(form_terminal_center, 1.00, 0.88, 0.78, 1.66, 0.14, mat_index["M_RC_Gunmetal"])
    form_terminal_acc.add_box((6.12, -1.96, 1.70), (1.12, 0.98, 0.34), mat_index["M_RC_Yellow"], rotation=(-0.18, 0.0, 0.0))
    form_terminal_acc.add_box((6.12, -2.40, 1.79), (0.56, 0.09, 0.26), mat_index["M_RC_Cyan"], rotation=(-0.18, 0.0, 0.0))
    create_mesh_object("SM_RobotCenter_FormTerminal", form_terminal_acc, collections["04_TransformStation"], materials, material_order, (6.12, -1.92, 0.78), bevel=0.08)

    # 08 Exactly two thicker, readable system links. No third cable is introduced.
    create_curve(
        "SM_RobotCenter_Link_Shop",
        [(-1.72, 1.15, 1.72), (-2.08, 1.12, 1.58), (-2.48, 0.90, 1.42), (-2.32, 0.62, 1.38)],
        collections["05_Detail"], materials["M_RC_DarkRubber"], bevel_depth=0.17,
    )
    create_curve(
        "SM_RobotCenter_Link_Transform",
        [(1.72, 1.15, 1.72), (1.54, 0.98, 1.56), (1.36, 0.62, 1.36), (1.26, 0.22, 1.30)],
        collections["05_Detail"], materials["M_RC_DarkRubber"], bevel_depth=0.17,
    )
    ...
    # Robot-only exit ramp: broad, gentle, open-edged, and independent from the player entrance.
    exit_ramp_acc = MeshAccumulator()
    x_outer, x_inner = -9.55, -6.70
    exit_y = -1.75
    z_outer, z_inner = 0.10, 0.78
    exit_width = 2.80
    exit_ramp_acc.add_x_ramp_wedge(x_outer, x_inner, exit_y, exit_width, z_outer, z_inner, mat_index["M_RC_Gunmetal"])
    exit_slope_length = math.sqrt((x_inner - x_outer) ** 2 + (z_inner - z_outer) ** 2)
    exit_slope_angle = math.atan2(z_inner - z_outer, x_inner - x_outer)
    for y in (exit_y - exit_width * 0.5 + 0.08, exit_y + exit_width * 0.5 - 0.08):
        exit_ramp_acc.add_box(
            ((x_outer + x_inner) * 0.5, y, (z_outer + z_inner) * 0.5),
            (exit_slope_length, 0.16, 0.14), mat_index["M_RC_Yellow"],
            rotation=(0.0, -exit_slope_angle, 0.0),
        )
    exit_ramp = create_mesh_object(
        "SM_RobotCenter_RobotExitRamp", exit_ramp_acc, collections["01_Base"],
        materials, material_order, (x_outer, exit_y, 0.0), bevel=0.06,
    )
    exit_ramp["RC_RampWidthMeters"] = exit_width
    exit_ramp["RC_RampGradeDegrees"] = math.degrees(exit_slope_angle)
    exit_ramp["RC_Function"] = "Robot-only exit to external navigation"

    create_reference_proxies(collections["00_Reference"], materials)
    create_empty("ShopInteractPoint", (-3.30, -1.02, 0.82), collections["00_Reference"])
    create_empty("FormInteractPoint", (6.12, -2.82, 0.82), collections["00_Reference"])
    create_empty("ShopRobotDisplayPoint", (-5.15, -1.75, 1.43), collections["00_Reference"])
    create_empty("RobotSpawnPoint", (-5.15, -1.75, 1.43), collections["00_Reference"])
    create_empty("RobotExitPoint", (-6.88, -1.75, 0.82), collections["00_Reference"])
    create_empty("RobotJoinNavPoint", (-9.72, -1.75, 0.12), collections["00_Reference"])
    ...
    shop_path = create_curve(
        "REVIEW_Path_RobotExit",
        [(-5.15, -1.75, 1.55), (-6.88, -1.75, 1.02), (-8.10, -1.75, 0.52), (-9.72, -1.75, 0.25)],
        collections["00_Reference"], materials["M_RC_Orange"], bevel_depth=0.10,
    )
    shop_path["RC_ExportCandidate"] = False
    shop_path["RC_ReviewOnly"] = True
    shop_path.hide_render = True
    transform_path = create_curve(
        "REVIEW_Path_TransformApproach",
        [(0.0, -3.35, 0.90), (1.20, -3.00, 0.96), (2.25, -2.30, 1.08), (3.32, -1.12, 1.45)],
        collections["00_Reference"], materials["M_RC_Cyan"], bevel_depth=0.09,
    )
    transform_path["RC_ExportCandidate"] = False
    transform_path["RC_ReviewOnly"] = True
    transform_path.hide_render = True
    create_preview_setup(scene, collections["00_Reference"], materials)

    root["RC_Asset"] = "Robot Center"
    root["RC_Phase"] = "P03_LayoutOptimization"
    ...
    root["RC_LayoutIntent"] = "Separate player operation, robot deployment/exit, and open three-node transformation access"
