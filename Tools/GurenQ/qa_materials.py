...
cases=[('interrupt_radiation',0,480)]
 ...
 try:
  if state['phase']=='next':
   ...
   if state['i']>=len(cases):
    u.SystemLibrary.execute_console_command(w,'t.MaxFPS 60');u.unregister_slate_post_tick_callback(builtins.q_cases_tick);(O/'qa_materials.json').write_text(json.dumps(results,indent=2));print('Q_CASES_DONE');return
   ...
   if (not q.is_q_active() and elapsed>.7) or elapsed>8.5:
    ...
    if name=='interrupt_radiation':result['original']=state['original_material'];result['after']=str(targets[0].get_editor_property('body').get_material(0));result['material_restored']=str(targets[0].get_editor_property('body').get_material(0))==state['original_material'];result['detached']=targets[0].get_attach_parent_actor() is None
    results.append(result);(O/'qa_materials_partial.json').write_text(json.dumps(results,indent=2));state['phase']='next'
