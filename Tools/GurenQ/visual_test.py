...
p.get_component_by_class(u.SpringArmComponent).set_editor_property('target_arm_length',1250)
...
def visual_sample(dt):
 try:
  ...
  if elapsed>5.8:
   u.unregister_slate_post_tick_callback(builtins.q_visual_tick);u.GameplayStatics.set_global_time_dilation(w,1);(O/'visual_test.json').write_text(json.dumps(state['rows'],indent=2));print('Q_VISUAL_DONE')
