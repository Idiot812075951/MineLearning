...
- 任务开始前，先完成 Unreal MCP 握手及一次成功的只读工具调用，并确认连接的是目标项目。插件已勾选、端口开放或用户说“已启动”不能代替真实连接验证。
- 所有 UE 资产操作（包括查询、创建、修改、编译、保存和删除 Blueprint、UMG、材质、关卡等资产）必须通过 MCP 执行；不得通过离线资产脚本、直接改写资产文件或模拟编辑器输入绕过 MCP。通过 MCP 提供的编辑器工具执行操作属于 MCP 通道。
- MCP 连接中断不等于整个任务中断。报告故障，继续已授权的连接诊断、重连、必要的编辑器重启，以及不依赖 UE 资产访问的代码、配置或文档工作。连接恢复并经只读调用确认目标项目后，继续资产操作。
- 对断连时结果不确定的操作，重连后先检查实际状态，再决定是否重试，避免重复创建或覆盖。工具参数错误、工具不存在或资产编译失败不等于连接失败。
- 不得把手动 StartServer 当成持久修复，也不得只因插件启用就宣称下次启动会自动连接。
