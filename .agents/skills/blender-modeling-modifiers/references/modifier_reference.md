# Blender Modifier Reference — Upstream Catalog with MineLearning Corrections

Upstream source: [modifier_reference.md at 78e9151](https://github.com/ra100/blender-claude-plugin/blob/78e9151fdc9e01ce37f1d16a9b677c3047411885/skills/blender-modeling-modifiers/references/modifier_reference.md).

This catalog is not authoritative for the connected runtime. Verify the selected modifier in Blender 5.1.2 RNA/docs before use. Adding, removing, reordering, or applying a modifier is a write and remains subject to `../SKILL.md`.
...
| Boolean                | `'BOOLEAN'`        | Boolean operations between meshes             | `operation` ('INTERSECT', 'UNION', 'DIFFERENCE'), `object`, `solver` (`FLOAT`, `EXACT`, `MANIFOLD` in inspected 5.1.2), `use_self`, `use_hole_tolerant`, `operand_type`, `collection` |
...
| Weighted Normal | `'WEIGHTED_NORMAL'` | Calculates normals based on face area/angle | `mode` ('FACE_AREA', 'CORNER_ANGLE', 'FACE_AREA_AND_ANGLE'), `weight`, `keep_sharp`, `thresh`; inspected 5.1.2 has no `face_influence` property |
