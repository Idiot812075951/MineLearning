...
Read `project-compatibility.md` before use. These are independent recipes, not a script to run end-to-end. Resolve exact targets and authorize each mutation. Never run clear/remove/new-rig examples on protected production data. Preserve current rotation modes and use the corresponding Euler, quaternion or axis-angle property; do not change modes just to make an example run.
...
### Bone Keyframes (Object or Pose Mode)
...
# Use the existing rotation mode; don't key an inactive rotation property.
if pose_bone.rotation_mode == 'QUATERNION':
    pose_bone.rotation_quaternion = (1, 0, 0, 0)
    pose_bone.keyframe_insert(data_path="rotation_quaternion", frame=1)
elif pose_bone.rotation_mode == 'AXIS_ANGLE':
    pose_bone.rotation_axis_angle = (0.5, 0, 0, 1)
    pose_bone.keyframe_insert(data_path="rotation_axis_angle", frame=1)
else:
...
### Accessing FCurves (Historical API — Not for Blender 5.x)

The following historical `action.fcurves` example is retained for recognizing old scripts. Do not use it for the project's Blender 5.x files; use the slot-aware helper below.
...
### Accessing FCurves (Layered Actions — Blender 4.4+/5.x)

Layered Actions use `action.layers[].strips[].channelbags[].fcurves`. Multiple objects can use different slots of one Action, and a slot can serve compatible data-blocks. Scope edits by the assigned slot, not by traversing every channelbag indiscriminately.
...
# Read-only inventory across all slots; not a bulk mutation target.
...
# Helpers return only the assigned slot's curves. Missing/ambiguous data fails closed.
def get_slot_fcurves(obj):
    ad = obj.animation_data
    if not ad or not ad.action or not ad.action_slot:
        raise ValueError('Object has no assigned Action and slot')
    curves = []
...
def find_slot_fcurve(obj, data_path, index=0):
    matches = [fc for fc in get_slot_fcurves(obj)
               if fc.data_path == data_path and fc.array_index == index]
    if len(matches) != 1:
        raise ValueError('Expected one curve; inspect layers/strips before editing')
    return matches[0]

# Separate mutation example, only after confirming target compatibility and backup.
# Preserve originals; select the corresponding slot by identifier, never write a handle.
source_slot_identifier = obj.animation_data.action_slot.identifier
new_action = action.copy()
new_action.use_fake_user = True
copied_slot = next(s for s in new_action.slots if s.identifier == source_slot_identifier)
target_obj.animation_data_create()
target_obj.animation_data.action = new_action
target_obj.animation_data.action_slot = copied_slot
# Validate target data paths and playback before treating this as retargeting.
```

### Modifying Keyframe Points

```python
fc = find_slot_fcurve(obj, "location", index=0)
...
```python
fc = find_slot_fcurve(obj, "location", index=2)  # Z location
...
Removal is intentionally not bundled with creation. Do not remove original or historical Actions during routine editing. Assign a suitable Action slot when binding layered Actions, as shown above.
...
bone.inherit_scale = 'FULL'  # FULL, FIX_SHEAR, ALIGNED, NONE, AVERAGE, NONE_LEGACY
# Use armature Bone Collections, not the removed 32-layer bone.layers API.
...
bone.bbone_curveinz = 0.0       # In handle Z curve
bone.bbone_curveoutx = 0.0      # Out handle X curve
bone.bbone_curveoutz = 0.0      # Out handle Z curve
...
# Matrix setters exist, but head/tail are read-only evaluated coordinates.
...
```python
# Data API works in Object or Pose mode; no mode change needed here.
pose_bone = arm_obj.pose.bones["Bone"]
...
### Set Interpolation for a Deliberately Selected Slot

Only on a disposable/approved authoring Action. Further restrict paths and frames for narrow edits; do not apply wholesale to existing baked production Actions.

```python
for fcurve in get_slot_fcurves(obj):
...
This is only a parameter illustration. First isolate the intended selection (including pose-bone selection), save the original active Action/slot/NLA state and choose a recoverable destination. For skeletal channels use `POSE`. Inspect the newly created Action/slot and validate with authoring controls inactive; do not clear original constraints or parents to make a bake appear independent.
...
Clearing animation data removes assignments, NLA and drivers; it is not a routine repair. Only perform a separately authorized, exact-target cleanup with a recoverable baseline. Temporarily unassigning an Action for an inspection requires restoring its original slot and NLA state afterward.
