# Blender Constraints - Selected Reference

A lookup table, not an exhaustive or version-independent API contract. Read `project-compatibility.md` first and confirm the relevant constraint's RNA fields/enums in the current Blender build. Slash notation in property cells is shorthand, not a literal property name. Do not add constraints to a locked production rig just because a recipe appears below.
...
# Bone constraints (Object or Pose mode via data API)
...
| Child Of    | `'CHILD_OF'`    | Dynamic parent-child relationship                         | `target`, `subtarget`, `use_location_x/y/z`, `use_rotation_x/y/z`, `use_scale_x/y/z`, `inverse_matrix`, `set_inverse_pending`; verify inverse setup in the intended owner/target spaces |
...
| Constraint         | Type String            | Description                | Key Properties                                                                                                                                                                                                   |
| ------------------ | ---------------------- | -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Inverse Kinematics | `'IK'` | IK solver for bone chains  | `target`, `subtarget`, `pole_target`, `pole_subtarget`, `pole_angle`, `chain_count`, `use_rotation`, `ik_type` ('COPY_POSE', 'DISTANCE'), `iterations`, `use_location`, `use_stretch`, `weight`, `orient_weight` |
...
Common spaces are listed below. Inspect the specific property enum and its applicability to object/bone targets. `LOCAL_OWNER_ORIENT` is target-only in the inspected Blender 5.1.2 RNA, not an owner-space enum.
...
| `'LOCAL_OWNER_ORIENT'` | Target-only local space with owner orientation |
...
These drivers only demonstrate blending influences. A production switch additionally requires exact IK/FK pose matching, consistent spaces and pole alignment to avoid popping; reuse existing controls and verify the transition before keying a switch.
...
Only for an explicitly stretchable auxiliary chain. This pattern is not suitable for Guren's rigid, length-locked production bones. Use non-stretch IK/tracking and actual telescoping parts for rigid mechanisms.
